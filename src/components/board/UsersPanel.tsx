...
const UsersPanel: React.FC<UsersPanelProps> = ({
  ...
  const toggleEditor = async (userId: string) => {
    ...
    setBoard({
      ...board,
      permissions: { ...board.permissions, editors: newEditors },
    });
    setUpdating(null);
  };
        ...
        {users.map((user) => (
          ...
          >
            ...
            {isAdmin && user.id !== board?.adminId && (
              ...
              >
                {board?.permissions?.editors?.includes(user.id)
                  ? "Can Edit"
                  : "View Only"}
              </button>
            )}
            <div
              className={`w-2 h-2 rounded-full ${user.isOnline ? "bg-green-500" : "bg-gray-500"}`}
            />
          </div>
        ))}
      </div>
