...
const CanvasBoard: React.FC<CanvasBoardProps> = ({
  ...
  const isPointInEraserRange = useCallback((point: Point, eraserPoint: Point, eraserSize: number): boolean => {
  ...
  const doesStrokeIntersectEraser = useCallback((strokePoints: Point[], eraserPoints: Point[], eraserSize: number): boolean => {
  ...
  const eraseElements = useCallback(async (eraserPoints: Point[], eraserSize: number) => {
    const elementsToDelete: string[] = [];
    
    elements.forEach(element => {
      if (element.id && element.type === 'pen') {
        // Check if this pen stroke intersects with the eraser path
        if (doesStrokeIntersectEraser(element.points, eraserPoints, eraserSize)) {
          elementsToDelete.push(element.id);
        }
      } else if (element.id && (element.type === 'rectangle' || element.type === 'circle')) {
        ...
        for (const eraserPoint of eraserPoints) {
          if (eraserPoint.x >= minX - eraserSize && eraserPoint.x <= maxX + eraserSize &&
              eraserPoint.y >= minY - eraserSize && eraserPoint.y <= maxY + eraserSize) {
            elementsToDelete.push(element.id);
            break;
          }
        }
  ...
  const setupCanvas = useCallback(() => {
    ...
    if (newScaleX <= 0 || newScaleY <= 0 || !isFinite(newScaleX) || !isFinite(newScaleY)) {
      setupTimeoutRef.current = setTimeout(setupCanvas, 100);
      return;
    }
  ...
  const screenToVirtual = useCallback((clientX: number, clientY: number): Point => {
    const container = containerRef.current;
  ...
  const virtualToScreen = useCallback((point: Point): Point => {
    return {
  ...
  const drawPath = useCallback((ctx: CanvasRenderingContext2D, points: Point[], color: string, width: number) => {
    ...
    if (points.length === 1) {
      ...
      ctx.arc(screenPoint.x, screenPoint.y, (width * scaleX) / 2, 0, Math.PI * 2);
      ctx.fill();
    } else {
  ...
  const drawShape = useCallback((ctx: CanvasRenderingContext2D, element: Element) => {
  ...
  const drawCursor = useCallback((ctx: CanvasRenderingContext2D, userId: string, position: Point) => {
  ...
  const drawEraserPreview = useCallback((ctx: CanvasRenderingContext2D, position: Point, size: number) => {
    const screenPos = virtualToScreen(position);
  ...
  const render = useCallback(() => {
    ...
    ctx.fillRect(offsetX, offsetY, VIRTUAL_WIDTH * scaleX, VIRTUAL_HEIGHT * scaleY);
    ...
    if (previewElement && previewElement.type !== 'eraser') {
      if (previewElement.type === 'pen') {
        drawPath(ctx, previewElement.points, previewElement.color, previewElement.strokeWidth);
      } else {
    ...
    animationFrameRef.current = requestAnimationFrame(render);
  }, [elements, previewElement, cursors, eraserPosition, currentTool, strokeWidth, scaleX, scaleY, offsetX, offsetY, drawPath, drawShape, drawCursor, drawEraserPreview]);

  // Database commit function
  const commitElement = useCallback(async (elementData: Partial<Element>) => {
    try {
  ...
  const handleStart = useCallback((point: Point) => {
    ...
    isDirty.current = true;
  }, [canEdit, currentTool, strokeColor, strokeWidth, fill, currentUserId, eraseElements]);
  ...
  const handleMove = useCallback((point: Point) => {
    ...
    } else if ((currentTool === 'rectangle' || currentTool === 'circle') && startPoint) {
      setPreviewElement({
        type: currentTool,
    ...
    isDirty.current = true;
  }, [isDrawing, currentStroke, currentTool, startPoint, strokeColor, strokeWidth, fill, currentUserId, eraseElements]);

  // Modified handleEnd - no eraser elements stored
  const handleEnd = useCallback((endPoint?: Point) => {
    if (!isDrawing) return;
    ...
    if (currentTool === 'pen') {
      ...
      setCurrentStroke([]);
    } else if ((currentTool === 'rectangle' || currentTool === 'circle') && startPoint && endPoint) {
      commitElement({
        type: currentTool,
    ...
    setStartPoint(null);
    isDirty.current = true;
  }, [isDrawing, currentStroke, currentTool, startPoint, strokeColor, strokeWidth, fill, commitElement]);
      ...
      elements.forEach(element => {
        if (element.type === 'pen') {
          ...
          if (element.fill) {
            ...
            exportCtx.fillRect(start.x, start.y, end.x - start.x, end.y - start.y);
          }
