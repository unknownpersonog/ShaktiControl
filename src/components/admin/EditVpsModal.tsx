...
const EditVpsModal = ({ vps, setShowEditVpsModal, fetchVpsList }: any) => {
  ...
  const handleEditVps = async () => {
    try {
      await makeRequest('PUT', `/api/uvapi/vps/edit/${updatedVps.id}`, updatedVps);
      fetchVpsList();
      ...
      <div className="bg-gray-800 p-6 w-full max-w-md rounded-lg border border-purple-500">
        ...
        {['name', 'os', 'port', 'owner', 'plan', 'user', 'ip'].map((field) => (
          <input
            ...
            onChange={(e) => setUpdatedVps({ ...updatedVps, [field]: e.target.value })}
        ...
        <div className="mt-4 flex justify-end space-x-2">
          <button onClick={() => setShowEditVpsModal(false)} className="px-4 py-2 bg-gray-600 text-white rounded-lg">Cancel</button>
          <button onClick={handleEditVps} className="px-4 py-2 bg-green-500 text-white rounded-lg">Save Changes</button>
        </div>
      </div>
    </div>
  );
